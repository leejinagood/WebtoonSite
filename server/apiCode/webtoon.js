    ...
    server.get('/api/webtoons', async (req, res) => {
        ...
        try {
            ...
            if (value) {
                ...
                const LikeKey = `likes:${row.webtoonID}`; //likes 키값 설정
                let LikeValue = await redisClient.get(LikeKey);
                if(LikeValue){ // 존재하면
                    res.send(JSON.parse(LikeValue)); // 문자열로 파싱
                }else{ //아니면 
                    for (const item of row) { // row의 갯수만큼 좋아요의 갯수를 담은 value 생성
                        const LikeKey = `likes:${item.webtoonID}`;
                        const totalLikesValue = item.totalLikes; 
                        await redisClient.set(LikeKey, totalLikesValue);  //저장
                    }
                }
