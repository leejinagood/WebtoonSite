...
export const insertComment = async (webtoonName, episodeNumber, userEmail, commentContent) => {
  try {
    const response = await fetch('http://3.37.36.238:4000/api/comment_insert', {
      method: 'POST',
      headers: {
...
export const loadCommentList = async (webtoonName, episodeNumber) => {
  try {
    const response = await fetch(`http://3.37.36.238:4000/api/comment?EnName=${webtoonName}&ep=${episodeNumber}`);
